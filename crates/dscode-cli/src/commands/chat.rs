...
//
// Only triggers near the 1M limit. Delegates summarization to
// deepseek-v4-flash so recent context stays fully intact while
// old turns become a compact summary.  Preserves prefix cache
// until the compaction event itself — which is rare.

/// Compact via LLM summary when approaching context limit.
/// Keeps last 3 full turns, replaces older messages with a
/// single system-style summary generated by deepseek-v4-flash.
async fn compact_via_llm(
    ...
    // Keep last 6 messages (~3 turns) fully intact
    const KEEP: usize = 6;
    ...
    // Clone old section — don't touch messages until LLM succeeds
    ...
    let mut summary_prompt = String::from(
        "Summarize this conversation concisely in Chinese. Preserve:\n\
         ...
         - Enforce: single files exceeding 400 lines must be split\n\n\
         Conversation:\n",
    );
    for m in &old_msgs {
        let role = &m.role;
    ...
    let data: serde_json::Value = match resp.json().await {
        ...
        Err(e) => {
            if narrow {
                eprintln!("\x1B[33m─ compaction parse failed: {e}\x1B[0m");
            }
            return false;
        }
    };
    ...
    let summary = match data["choices"][0]["message"]["content"].as_str() {
        ...
        None => {
            if narrow {
                eprintln!("\x1B[33m─ compaction returned empty\x1B[0m");
            }
            return false;
        }
    };

    // Safe to mutate messages now — LLM call succeeded
...
pub async fn run(args: &ChatArgs) {
    ...
    loop {
        ...
        // LLM compaction: summarize old turns via flash model when
        // approaching the 1M context limit.
        ...
        // Default system prompt with coding discipline rules.
        // Merges with AGENT.md and --system if provided.
...
- Only reference real file paths, function names, and APIs that actually exist.
- Do not invent library functions, crate versions, or API endpoints.

## Code Quality
- Single files exceeding 400 lines must be split into smaller modules (encapsulate, reuse).
- Prefer small, focused, composable modules over monolithic files.
- Keep functions short and single-responsibility.
- Refactor without changing behavior. Do not add speculative edge case handling.";
        // AGENT.md takes precedence, then --system, then built-in default
        let agent_prompt = api::load_agent_md();
        if let Some(ref ap) = agent_prompt {
            api_msgs.push(serde_json::json!({"role": "system", "content": format!("{}\n\n{}", default_system, ap)}));
        } else if let Some(sp) = &args.system {
            if !sp.is_empty() {
                api_msgs.push(serde_json::json!({"role": "system", "content": format!("{}\n\n{}", default_system, sp)}));
            }
        } else {
            api_msgs.push(serde_json::json!({"role": "system", "content": default_system}));
        }
                ...
                Ok(stream_res) => {
                    ...
                    if !stream_res.tool_calls.is_empty() {
                        ...
                        for tc in &stream_res.tool_calls {
                            ...
                            // V4 has 1M context — keep tool results intact up to 120K chars
                            const MAX_TOOL_CHARS: usize = 120_000;
