...
impl AgentEngine {
 ...
 let turn_start = std::time::Instant::now();
        ...
        // Helper: update status bar at top of screen
        let print_status = |status: &str| {
            if !options.silent && std::io::stdout().is_terminal() {
                print!("\x1B[s\x1B[H\x1B[2K{}\x1B[u", status);
        ...
        loop {
            ...
            while stream_res.finish_reason.as_deref() == Some("length") && stream_res.tool_calls.is_empty() {
                // Update top bar (all terminals)
                print_status("\x1B[90m─ continuing...\x1B[0m");
        ...
        // Show final status at top of (replaces any "continuing..." or other bar)
        {
            ...
            let status = if options.narrow {
                format!("\x1B[90m│{} \x1B[90m{:.0}s {:.0} tok\x1B[0m│", symbol, elapsed.as_secs_f64(), total_tok)
            } else {
                format!("\x1B[90m── {} {} · {:.1}s · {} tok\x1B[90m ──\x1B[0m",
